...
export async function POST(request: Request) {
  ...
  const { slug, kind, method } = parsed.data;
  ...
  if (kind === "signal") {
    ...
    description = `TradingIA Signals — ${plan.name.en} (monthly)`;
  } else {
    ...
    description = `TradingIA — ${product.name}`;
  }
  ...
  const result = await createCryptoInvoice({
    ...
    description,
    successUrl: `${base}/${locale}/checkout/success`,
    cancelUrl,
  });
  if ("error" in result) {
    return NextResponse.json(
